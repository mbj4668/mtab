...
-type col() :: #{
    ...
    % default is dynamically calculated
    width => pos_integer()
}.
...
mk_style(M) ->
    D = #style{},
    #style{
        spacer = maps:get(spacer, M, D#style.spacer),
...
mk_sep0(M) ->
    D = #sep{},
    #sep{
        left = maps:get(left, M, D#sep.left),
...
mk_row0(M) ->
    D = #row{},
    #row{
        left = maps:get(left, M, D#row.left),
